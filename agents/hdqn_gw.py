...
class MetaController(nn.Module):
    # sates are represented from coordinates to one hot vector 
    def __init__(self, in_features=30, out_features=30):
        """
        Initialize a Meta-Controller of Hierarchical DQN network for the diecreate Grid World experiment
            in_features: current state tuple.
            out_features: goal state tuple.
        """
...
class Controller(nn.Module):
    ...
    def __init__(self, in_features=60, out_features=4):
        """
        Initialize a Controller(given goal) of h-DQN for the diecreate grid world experiment
            in_features: state+goal state.
